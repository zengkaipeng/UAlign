...
from sparse_backBone import GINBase
import torch

MODEL_PATH, TOKEN_PATH = 'MODEL.pth', 'TOKENIZER.pkl'
HEADS, ENCODER_LAYER, DECODER_LAYER = 8, 10, 8
BACKBONE, DIM, DEVICE = 'GIN', 288, 0
...
GNN = GINBase(
    num_layers=ENCODER_LAYER, dropout=0.1,
    embedding_dim=DIM, edge_last=True, residual=True
)
...
print(greedy_inference_one(
    model, tokenizer, graph_input, device,
    max_len=200, begin_token='<CLS>', end_token='<END>'
))
